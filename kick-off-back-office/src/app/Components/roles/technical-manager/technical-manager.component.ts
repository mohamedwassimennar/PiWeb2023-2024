...
@Component({
  selector: 'app-technical-manager',
  templateUrl: './technical-manager.component.html',
  styleUrls: ['./technical-manager.component.css']
})
export class TechnicalManagerComponent  implements OnInit{
  users: User[] = [];
  showDetails: boolean[] = [];
  ...
  ngOnInit(): void {
    this.loadtechnicalManagerUsers();
  }


  loadtechnicalManagerUsers() {
    this.userService.getAllUsersByRole('technicalManager').subscribe(users => {
      this.users = users;
    });
  }
