...
function navbarColorOnResize() {
  const sidenav = document.getElementById('sidenav-main');
  if (!sidenav) return; // Check if sidenav exists, exit function if not

  if (window.innerWidth > 1200) {
    const referenceButtons = document.querySelector('[data-class]');
    if (!referenceButtons) return; // Check if referenceButtons exists, exit function if not

    const sidenavClassList = sidenav.classList;

    if (referenceButtons.classList.contains('active') && referenceButtons.getAttribute('data-class') === 'bg-transparent') {
      sidenavClassList.remove('bg-white');
    } else {
      if (!document.body.classList.contains('dark-version')) {
        sidenavClassList.add('bg-white');
      }
    }
  } else {
    sidenav.classList.add('bg-white');
